-- Add kwp_kw column to hero_dashboard_projects materialized view.
-- kWp is extracted from measure_name via regex (e.g. "5,2 kWp", "10.0kWp", "8 KWP").
-- This approach works without Hero API changes; if Hero adds a native kwp field
-- later, the projects sync can promote it and this view can be updated to prefer it.
...
SELECT
  ...
  -- kWp extracted from measure_name: matches patterns like "5.2 kWp", "10,0kWp", "8 KWP"
  (
    NULLIF(
      REGEXP_REPLACE(
        REGEXP_REPLACE(
          LOWER(COALESCE(p.measure_name, '')),
          '^.*?([0-9]+[.,][0-9]+|[0-9]+)\s*kwp.*$', '\1'
        ),
        ',', '.'
      ),
...
CREATE INDEX IF NOT EXISTS hero_dashboard_projects_kwp_idx
  ON public.hero_dashboard_projects (kwp_kw)
  WHERE kwp_kw IS NOT NULL;
